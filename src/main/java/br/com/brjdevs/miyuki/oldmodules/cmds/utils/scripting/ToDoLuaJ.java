...
package br.com.brjdevs.miyuki.David.oldmodules.cmds.utils.scripting;
...
public class ToDoLuaJ {
	...
	public static void main(String[] args) {
		// Create server globals with just enough library support to compile userTimeout scripts.
		server_globals = new Globals();
		server_globals.load(new JseBaseLib());
		server_globals.load(new PackageLib());
		server_globals.load(new StringLib());

		// To add scripts, we occasionally need a math library in addition to compiler support.
	...
	static void runScriptInSandbox(String script) {
		...
		// user_globals.add(new LuajavaLib());
		...
		// However we don't wish to expose the library in the userTimeout globals,
		// so it is immediately removed from the userTimeout globals once created.
